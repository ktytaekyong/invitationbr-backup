      ...
      <div className={styles.manage__item_wrap}>
            ...
            <FormControlLabel
              ...
              control={
                <Switch
                  ...
                  onChange={() =>
                    onChange((prev) =>
                      prev.map((item, idx) => (inviteIdx === idx ? { ...item, use: !item.use } : item))
                    )
                  }
                />
              }
        ...
        <div className={styles.manage__item_date}>
          <div className={styles.manage__item_date_wrap}>
            <p className={styles.manage__item_date_delete}>
              {Number(DDayCalculator(inviteItem.weddingDate)) + 30}일 남음
            </p>
            <p className={styles.manage__item_date_day}>
              {DateConverterDot(AddDaysToDate(inviteItem.weddingDate, 30))}까지
            </p>
          </div>
          <Link to="/Produce/Preview">
            <span>청첩장 보기</span>
        ...
        <div className={styles.manage__item_content}>
          ...
          <div className={styles.manage__item_content_info}>
            <p>
              최초 작성일<span>{inviteItem.createDate}</span>
            </p>
            <p>
              최종 수정일<span>{inviteItem.modifyDate}</span>
            </p>
            <p>
              삭제 예정일<span>{AddDaysToDate(inviteItem.weddingDate, 30)}</span>
            </p>
            <Button content="참석 여부 확인" styleType="manage" onClick={() => navigate("/Manager/Detail")}></Button>
          </div>
        </div>
        <ButtonWrapper>
          <Button content="복사" styleType="manage__cancel"></Button>
          <Button content="청첩장 편집하기" onClick={() => navigate("/Produce")} styleType="manage__point" />
        </ButtonWrapper>
      </div>
